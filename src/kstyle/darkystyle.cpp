...
#include "darkystyle.h"

#include "darky.h"
#include "darkyanimations.h"
#include "darkyblurhelper.h"
#include "darkyframeshadow.h"
#include "darkymdiwindowshadow.h"
#include "darkymnemonics.h"
#include "darkypropertynames.h"
#include "darkyshadowhelper.h"
#include "darkysplitterproxy.h"
#include "darkystyleconfigdata.h"
#include "darkytoolsareamanager.h"
#include "darkywidgetexplorer.h"
#include "darkywindowmanager.h"
...
#if DARKY_HAVE_QTQUICK
#include <QQuickWindow>
#endif

// #include <QDebug>

namespace DarkyPrivate
{
...
private:
    ...
    Darky::WeakPointer<const QWidget> _tabBar;
};
...
public:
    ...
    explicit ComboBoxItemDelegate(QAbstractItemView *parent)
        ...
        , _itemMargin(Darky::Metrics::ItemView_ItemMarginWidth)
    {
    }
...
private:
    //* proxy
    Darky::WeakPointer<QAbstractItemDelegate> _proxy;
...
Style::Style()
    ...
    , _tabBarData(new DarkyPrivate::TabBarData(this))
#if DARKY_HAVE_KSTYLE
    , SH_ArgbDndWindow(newStyleHint(QStringLiteral("SH_ArgbDndWindow")))
    , CE_CapacityBar(newControlElement(QStringLiteral("CE_CapacityBar")))
#endif
{
    // use DBus connection to update on darky configuration change
    auto dbus = QDBusConnection::sessionBus();
    dbus.connect(QString(),
                 QStringLiteral("/DarkyStyle"),
                 QStringLiteral("org.kde.Darky.Style"),
                 QStringLiteral("reparseConfiguration"),
                 this,
                 SLOT(configurationChanged()));

    dbus.connect(QString(),
                 QStringLiteral("/DarkyDecoration"),
                 QStringLiteral("org.kde.Darky.Style"),
                 QStringLiteral("reparseConfiguration"),
                 this,
                 SLOT(configurationChanged()));
    ...
    if (qobject_cast<QLabel*>(widget)) {
        ...
        while (parent) {
            if (parent->inherits("KMessageWidget")) {
                ...
                if (!parent->property("_darkyRoundedOverlay").isValid())
                {
                    ...
                    parent->setProperty("_darkyRoundedOverlay", QVariant::fromValue(static_cast<QObject*>(overlay)));
                }
                break;
            }
    ...
    if (qobject_cast<QScrollBar *>(widget)) {
        ...
        if (!hasParent(widget, "QWebView")) {
            ...
            if (itemView && itemView->itemDelegate() && itemView->itemDelegate()->inherits("QComboBoxDelegate")) {
                itemView->setItemDelegate(new DarkyPrivate::ComboBoxItemDelegate(itemView));
            }
        }
...
void Style::unpolish(QWidget *widget)
{
    ...
    if (DarkyPrivate::possibleTranslucentToolBars.contains(widget))
        DarkyPrivate::possibleTranslucentToolBars.remove(widget);
        ...
        if (widget && event->type() == QEvent::Paint) {
            if (widget->isWindow() && !_isKonsole && widget->testAttribute(Qt::WA_StyledBackground) && widget->testAttribute(Qt::WA_TranslucentBackground)) {
                ...
                case Qt::Sheet: {
                    ...
                    {
                        if( DarkyPrivate::possibleTranslucentToolBars.size() == 1 )
                        {
                            QSet<const QWidget *>::const_iterator i = DarkyPrivate::possibleTranslucentToolBars.constBegin();
...
QRect Style::progressBarGrooveRect(const QStyleOption *option, const QWidget *widget) const
{
    ...
    const bool horizontal(DarkyPrivate::isProgressBarHorizontal(progressBarOption));
...
bool Style::drawMenuBarEmptyAreaControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (widget && _helper->titleBarColor(windowActive).alphaF() * 100.0 < 100 && _translucentWidgets.contains(widget->window())) {
        ...
        if (DarkyPrivate::possibleTranslucentToolBars.isEmpty())
...
bool Style::isStylableToolbar(const QWidget *w, bool allowInvisible) const // should be in helper
{
    ...
    if (QMainWindow *mw = qobject_cast<QMainWindow *>(p)) {
        ...
        if (tb->orientation() == Qt::Vertical) {
            if (tb->y() == 0) {
                if (DarkyPrivate::possibleTranslucentToolBars.size() == 0) {
                    DarkyPrivate::possibleTranslucentToolBars.insert(w);
                    return true;
                } else if (DarkyPrivate::possibleTranslucentToolBars.contains(w) && DarkyPrivate::possibleTranslucentToolBars.size() == 1)
                    return true;
